mod sudokus;
use sudokus::{
    DfsBlock, SudokuGrid, BOTTOM_LEFT_BLOCK, BOTTOM_RIGHT_BLOCK, MIDDLE_CENTER_BLOCK,
    TOP_RIGHT_BLOCK,
};
...
pub fn generate(n: usize, m: usize) -> Box<[u8]> {
    ...
    for x in 0..n {
        ...
        for y in 0..m {
            ...
            if let Err(_) = dfs.next_solution(&mut sg) {
                ...
                other_dfs
                    ...
                    .unwrap_or_else(|_| panic!("Could not solve\n{sg:?}"));
                dfs.reset(&mut sg);
                // TODO: fix this - if possible? maybe I also need to try changing the other block
                while let Err(_) = dfs.next_solution(&mut sg) {
                    other_dfs
                        .next_solution(&mut sg)
                        .unwrap_or_else(|_| panic!("Could not solve\n{sg:?}"));
                    dfs.reset(&mut sg);
